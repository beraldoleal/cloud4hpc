...
\begin{abstract}
O uso de computação na nuvem por usuários de Computação de Alto Desmpenho (HPC - \emph{High Performance Computing}) ainda sofre muita resistência, em grande medida, por conta da grande degradação de desempenho que essas aplicações sofrem ao serem executadas na nuvem. Entre os principais fatores que levam à degradação, pode-se citar (1) interconexão de rede lenta, que geralmente utiliza \textit{hardware commodity}, (2) ambiente heterôgeneo com diferentes gerações de processadores, (3) compartilhamento do hardware entre diferentes usuários e (4) sobrecarga da virtualização.

Diferentes estratégias para viabilizar HPC na nuvem vem sendo discutidas e propostas. Em geral, pode-se classificar essas estratégias em \textit{Cloud-Aware}, quando a aplicação HPC tem ciência de que está sendo executada em um ambiente na nuvem e de suas limitações ou \textit{HPC-Aware}, que são estratégias que visam configurar a nuvem de forma otimizada para a execução de aplicações HPC. Nesta proposta, será apresentada uma solução que orbita as estratégias \textit{HPC-Aware}, em que baseando-se no padrão de comunicação da aplicação, as Máquinas Virtuais (VM - \textit{Virtual Machine}) serão realocadas em hospedeiros físicos com o objetivo de reduzir a sobrecarga inerente de um ambiente na nuvem para aplicações HPC. A hipótese é que, dado um determinado padrão de comunicação poderia-se deixar VMs que trocam mensagens constantemente mais próximas (no mesmo hospedeiro ou alguns \textit{hops} de distância). Uma das possibilidades que também deve ser avaliada é a alocação de VMs com padrões de comunicação incompatíveis entre si em máquinas físicas distintas e, dessa forma, reduzir a interferência causada entre elas.
\end{abstract}
...
\multirow{3}{*}{Pré-Pesquisa} & \begin{tabular}[c]{@{}c@{}}Qual o custo (sobrecarga) \\ para a aplicação HPC ao se reconfigurar a rede ?\end{tabular} \\ \cline{2-2} 
                              ...
                              & \begin{tabular}[c]{@{}c@{}}Quão rápido e preciso \\ é caracterizar uma troca de mensagem ?\end{tabular}        \\ \hline
...
\textbf{\begin{tabular}[c]{@{}c@{}}Latência de\\ Comunicação\end{tabular}}  & Micro Segundos ($\mu$s)                                                                        & \begin{tabular}[c]{@{}l@{}}Intervalo de tempo gasto entre \\ uma troca de mensagem.\end{tabular}                    \\ \hline
...
\begin{tabular}{|c|c|l|}
\hline
\textbf{Benchmark}                                                             & \textbf{\begin{tabular}[c]{@{}c@{}}(S)intético / \\ (C)arga Real\end{tabular}} & \multicolumn{1}{c|}{\textbf{Descrição}}                                                                     \\ \hline
\textbf{SPEC \cite{Muller:2010}}                                                                  & (S)                                                                            & Será utilizado o benchmark mpi2007.                                                                         \\ \hline
\textbf{NPB}                                                                   & (S)                                                                            & \begin{tabular}[c]{@{}l@{}}Diversos benchmarks sintéticos que \\ podem ser executados com MPI.\end{tabular} \\ \hline
\textbf{\begin{tabular}[c]{@{}c@{}}Parallel Workloads \\ Archive\end{tabular}} & (C)                                                                            & Traces de cargas de trabalho reais.                                                                         \\ \hline
\end{tabular}
\end{adjustbox}
